...
export class LocationsComponent implements OnInit { 
    ...
    this.userInputSubjectSubscription = this.userInpuSubject$.pipe(
      ...
      .pipe(
        switchMap(locationName => {                      
          this.enableOrDisableSubmitButton(locationName);     
  ...
  onKeyUp(){ 
    ...
    const trimmedLocationName = this.trimAtStart(this.locationName);    
  ...
  onClickOption(selectedLocationName){
    // Save selected option and display it on the user input textbox
    this.locationName = selectedLocationName;       
  } 

  // Removes the whitespaces at the beginning of a provided string and returns the string
  trimAtStart(text: string): string {
  ...
  // Sets submitButtonEnabled value as true or false
  enableOrDisableSubmitButton(trimmedLocationName){    
    this.submitButtonEnabled = trimmedLocationName.length > 1;
  }
